...
proc create_root_design { parentCell } {
  ...
  set_property -dict [ list CONFIG.BOARD.ASSOCIATED_PARAM {DIFFCLK_BOARD_INTERFACE}  ] $mgt_clk
  ...
  set_property -dict [ list CONFIG.BOARD.ASSOCIATED_PARAM {ETHERNET_BOARD_INTERFACE}  ] $sgmii
  ...
  set_property -dict [ list CONFIG.POLARITY {ACTIVE_HIGH}  ] $pma_reset_out
...
CONFIG.C_PHYADDR {1} CONFIG.C_PHY_TYPE {4} \
CONFIG.C_STATS {0} CONFIG.C_TYPE {1} \
CONFIG.ENABLE_LVDS {0} CONFIG.HAS_SGMII {true} \
CONFIG.MCAST_EXTEND {false} CONFIG.PHYRST_BOARD_INTERFACE {phy_reset_out} \
CONFIG.RXCSUM {None} CONFIG.RXMEM {32k} \
CONFIG.RXVLAN_STRP {false} CONFIG.RXVLAN_TAG {false} \
CONFIG.RXVLAN_TRAN {false} CONFIG.SIMULATION_MODE {false} \
CONFIG.TXCSUM {None} CONFIG.TXMEM {32k} \
...
CONFIG.TXVLAN_TRAN {false} CONFIG.USE_BOARD_FLOW {true} \
  ...
  set_property -dict [ list CONFIG.Data_Rate {1_Gbps} CONFIG.Enable_1588 {false} CONFIG.Enable_1588_1step {false} CONFIG.Enable_AVB {false} CONFIG.Enable_MDIO {true} CONFIG.Enable_Priority_Flow_Control {false} CONFIG.Frame_Filter {false} CONFIG.Half_Duplex {false} CONFIG.MAC_Speed {Tri_speed} CONFIG.Make_MDIO_External {false} CONFIG.Management_Interface {true} CONFIG.Number_of_Table_Entries {0} CONFIG.Physical_Interface {Internal} CONFIG.Statistics_Counters {false} CONFIG.Statistics_Reset {true} CONFIG.Statistics_Width {64bit} CONFIG.SupportLevel {0} CONFIG.Timer_Format {Time_of_day} CONFIG.USE_BOARD_FLOW {false}  ] $eth_mac

  # Create instance: pcs_pma, and set properties
  set pcs_pma [ create_bd_cell -type ip -vlnv xilinx.com:ip:gig_ethernet_pcs_pma:15.0 pcs_pma ]
  set_property -dict [ list CONFIG.Auto_Negotiation {true} CONFIG.C_PHYADDR {1} CONFIG.DIFFCLK_BOARD_INTERFACE {sgmii_mgt_clk} CONFIG.DrpClkRate {50.0} CONFIG.ETHERNET_BOARD_INTERFACE {sgmii} CONFIG.EXAMPLE_SIMULATION {0} CONFIG.Enable_1588 {false} CONFIG.Ext_Management_Interface {true} CONFIG.MDIO_BOARD_INTERFACE {mdio_mdc} CONFIG.Management_Interface {true} CONFIG.MaxDataRate {1G} CONFIG.Physical_Interface {Transceiver} CONFIG.RefClkRate {125} CONFIG.SGMII_Mode {10_100_1000} CONFIG.SGMII_PHY_Mode {false} CONFIG.Standard {SGMII} CONFIG.SupportLevel {Include_Shared_Logic_in_Core} CONFIG.Timer_Format {Time_of_day} CONFIG.TransceiverControl {false} CONFIG.USE_BOARD_FLOW {true}  ] $pcs_pma
